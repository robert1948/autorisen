...
## Modularization Notes (Current Status)

- We are incrementally migrating the backend from `backend/app` (monolith) to `backend/src/modules` (modular).
- First module extracted: `health` at `backend/src/modules/health/router.py` exposing `/alive` and `/ping`.
- The monolith app (`app.main:app`) now includes this module so `/alive` is served by the main container and Heroku dyno. Fallback logic still returns `{"status": "ok"}` if the module is not importable.
- CI added: `.github/workflows/ci-backend.yml` runs smoke tests for `/alive` and `/api/health`; `.github/workflows/ci-health.yml` verifies the standalone `services/health` app.
- Dev UX unchanged: `docker compose up`, `make migrate`, and `make smoke` continue to work.
