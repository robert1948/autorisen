...
class AuthTestSuite:
    ...
    async def test_profile_update(self):
        ...
        response = await self.client.put(
            "/api/auth/me", json=update_data, headers=headers
        )
    ...
    async def test_developer_earnings(self):
        ...
        response = await self.client.get(
            "/api/auth/developer/earnings", headers=headers
        )
        assert (
            response.status_code == 403
        ), "Customer should not access developer earnings"

        # Test access with developer role
        headers = {
            "Authorization": f"Bearer {self.tokens['developer']['access_token']}"
        }
        response = await self.client.get(
            "/api/auth/developer/earnings", headers=headers
        )
