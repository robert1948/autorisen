...
TEST_AI_DATA = {
    ...
    "context": {"page": "/test", "user_intent": "testing", "previous_interactions": []},
}
...
class IntegrationTestHelper:
    ...
    async def register_test_user(self, user_data: dict | None = None) -> dict[str, Any]:
...
class TestCapeAIWorkflows:
    ...
    async def test_ai_prompt_workflow(self):
        ...
        followup_request = {
            ...
            "context": {"page": "/dashboard", "user_intent": "feature_inquiry"},
    ...
    async def test_conversation_history_workflow(self):
        ...
        )  # Empty conversation returns 200 with empty messages
        data = response.json()
...
class TestEndToEndIntegration:
    ...
    async def test_complete_user_journey_workflow(self):
        ...
        followup_request = {
            ...
            "context": {"page": "/features", "user_intent": "feature_exploration"},
            "conversation_id": conversation_id,
        }
        ...
        print(
            f"✅ Step 8: Conversation history verified - {len(history_data['messages'])} messages"
        )
    ...
    async def test_concurrent_user_workflows(self):
        ...
        async def single_user_workflow(user_index: int):
            ...
            user_data = test_helper.create_test_user_data()
            user_data["email"] = (
                f"concurrent_user_{user_index}_{int(time.time())}@example.com"
            )
...
class TestPerformanceAndReliability:
    ...
    async def test_rate_limiting_behavior(self):
        ...
        print(
            f"✅ Rapid requests: {success_count} successful, {rate_limited_count} rate-limited"
        )
    ...
    async def test_data_consistency_workflow(self):
        ...
        print(
            f"✅ Conversation consistency verified: {len(history['messages'])} messages"
        )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        "Testing end-to-end API integration between Authentication V2 and CapeAI services"
    )
    print()
