...
class TestEnhancedConversation:
    ...
    async def test_create_thread(self, conversation):
        ...
        msg2 = await conversation.add_message(
            MessageRole.ASSISTANT, "Here's the answer"
        )

        # Create a thread
        thread = await conversation.create_thread(
            "Python Discussion", [msg1.message_id, msg2.message_id], "technical"
        )
    ...
    async def test_merge_threads(self, conversation):
        ...
        thread2 = await conversation.create_thread(
            "Thread 2", [msg2.message_id, msg3.message_id]
        )
    ...
    async def test_search_messages(self, conversation):
        ...
        await conversation.add_message(
            MessageRole.USER, "I need help with Python programming"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT, "Sure, what specifically about Python?"
        )
        await conversation.add_message(
            MessageRole.USER, "How do I handle async operations?"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT, "You can use asyncio for that"
        )
    ...
    async def test_generate_summary(self, conversation):
        ...
        await conversation.add_message(
            MessageRole.USER, "I'm working on a machine learning project using Python"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT, "Great! What kind of ML problem are you solving?"
        )
        await conversation.add_message(
            MessageRole.USER,
            "I'm building a recommendation system for an e-commerce platform",
        )
        await conversation.add_message(
            MessageRole.ASSISTANT,
            "Recommendation systems are fascinating. You could use collaborative filtering or content-based approaches",
        )
    ...
    async def test_generate_analytics(self, conversation):
        ...
        await conversation.add_message(
            MessageRole.USER, "Second message with more content"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT, "Detailed response with technical information"
        )
    ...
    async def test_export_conversation(self, conversation):
        ...
        await conversation.add_message(
            MessageRole.ASSISTANT, "Response for export test"
        )
    ...
    async def test_auto_threading_topic_based(self, conversation):
        ...
        await conversation.add_message(
            MessageRole.USER, "I want to learn about machine learning algorithms"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT,
            "ML algorithms are great! Let's start with supervised learning",
        )
        await conversation.add_message(
            MessageRole.USER, "Can you help me with web development using React?"
        )
        await conversation.add_message(
            MessageRole.ASSISTANT, "Sure! React is a powerful library for building UIs"
        )

        # Check if threads were created automatically
        assert (
            len(conversation.threads) >= 0
        )  # May or may not create threads based on keyword extraction
...
class TestConversationManager:
    ...
    async def test_get_user_conversations(self, manager):
        ...
        active_conversations = await manager.get_user_conversations(
            user_id, {"status": "active"}
        )
        assert len(active_conversations) == 2

        # Test filtering by type
        general_conversations = await manager.get_user_conversations(
    ...
    async def test_update_conversation(self, manager):
        ...
        updated_conv = await manager.update_conversation(
            conversation.conversation_id, updates
        )
    ...
    async def test_search_conversations(self, manager):
        ...
        await manager.add_message(
            ...
            MessageRole.USER,
            "Should I use NoSQL or SQL database?",
        )
    ...
    async def test_get_conversation_analytics(self, manager):
        ...
        # Add messages
        await manager.add_message(
            conversation.conversation_id, MessageRole.USER, "Question 1"
        )
        await manager.add_message(
            conversation.conversation_id, MessageRole.ASSISTANT, "Answer 1"
        )
        await manager.add_message(
            conversation.conversation_id, MessageRole.USER, "Question 2"
        )
    ...
    async def test_generate_conversation_summary(self, manager):
        ...
        # Add content
        await manager.add_message(
            conversation.conversation_id, MessageRole.USER, "I need help with Python"
        )
        await manager.add_message(
            conversation.conversation_id,
            MessageRole.ASSISTANT,
    ...
    async def test_export_conversations(self, manager):
        ...
        specific_export = await manager.export_conversations(
            user_id, [conv1.conversation_id], format="dict"
        )

        assert specific_export["conversation_count"] == 1
        assert (
            specific_export["conversations"][0]["conversation_id"]
            == conv1.conversation_id
        )
    ...
    async def test_get_system_analytics(self, manager):
        ...
        conv1 = await manager.create_conversation(
            user1, {"title": "Conv 1", "conversation_type": "general"}
        )
        conv2 = await manager.create_conversation(
            user1, {"title": "Conv 2", "conversation_type": "technical"}
        )
        conv3 = await manager.create_conversation(
            user2, {"title": "Conv 3", "conversation_type": "general"}
        )
    ...
    async def test_health_check(self, manager):
        ...
        await manager.add_message(
            conv.conversation_id, MessageRole.USER, "Test message"
        )
...
class TestPerformanceAndEdgeCases:
    ...
    async def test_concurrent_message_operations(self):
        ...
        async def add_messages(start_idx, count):
            for i in range(count):
                await conversation.add_message(
                    MessageRole.USER, f"Concurrent message {start_idx + i}"
                )
    ...
    async def test_empty_conversation_operations(self):
        ...
        conversation = EnhancedConversation(
            user_id="user_empty_test", conversation_data={"title": "Empty Test"}
        )
...
class TestIntegrationScenarios:
    ...
    async def test_complete_conversation_workflow(self):
        ...
        conversation = await manager.create_conversation(
            "user_integration", conversation_data
        )
        assert conversation is not None
        ...
        messages_to_add = [
            (
                MessageRole.USER,
                "I'm starting a new machine learning project for building a recommendation system. Where should I begin?",
            ),
            (
                MessageRole.ASSISTANT,
                "Great project choice! For recommendation systems, you'll want to consider collaborative filtering, content-based filtering, or hybrid approaches. What type of data do you have available?",
            ),
            (
                MessageRole.USER,
                "I have user purchase history, product descriptions, and user ratings. The dataset has about 100k users and 50k products.",
            ),
            (
                MessageRole.ASSISTANT,
                "Perfect! With that data, you could implement both collaborative and content-based filtering. I'd recommend starting with matrix factorization techniques like SVD or NMF for collaborative filtering.",
            ),
            (
                MessageRole.USER,
                "That sounds good. What about the technical stack? Should I use Python with scikit-learn or something more specialized?",
            ),
            (
                MessageRole.ASSISTANT,
                "For production systems, I'd suggest using Python with libraries like Surprise for collaborative filtering, scikit-learn for general ML tasks, and possibly TensorFlow or PyTorch for deep learning approaches.",
            ),
            (
                MessageRole.USER,
                "How do I handle the cold start problem for new users or products?",
            ),
            (
                MessageRole.ASSISTANT,
                "The cold start problem is common in recommendation systems. For new users, you can use popularity-based recommendations or demographic-based suggestions. For new products, content-based filtering using product features works well.",
            ),
        ]
        ...
        thread = await updated_conversation.create_thread(
            "ML Recommendation System Basics", ml_messages, "technical"
        )
        assert thread is not None
        ...
        export_data = await manager.export_conversations(
            "user_integration", [conversation.conversation_id], format="dict"
        )
    ...
    async def test_multi_user_conversation_management(self):
        ...
        for user in users:
            ...
            # Test filtering
            technical_conversations = await manager.get_user_conversations(
                user, {"type": "technical"}
            )
            assert len(technical_conversations) == 1
            assert (
                technical_conversations[0].conversation_type
...
def run_performance_benchmarks():
    ...
    async def benchmark_message_processing():
        ...
        print(
            f"Searched {len(conversation.messages)} messages in {search_duration*1000:.2f}ms"
        )
