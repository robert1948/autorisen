...
class TestVoiceService:
    ...
    async def test_get_voice_profiles(self, voice_service):
        ...
        google_profiles = await voice_service.get_voice_profiles(
    ...
    async def test_system_speech_to_text(
        self, mock_recognizer, voice_service, sample_audio_data
    ):
    ...
    async def test_openai_speech_to_text(
        self, mock_transcribe, voice_service, sample_audio_data
    ):
    ...
    async def test_speech_to_text_full_flow(self, voice_service, sample_audio_data):
        ...
        with patch.object(
            voice_service, "_perform_speech_recognition"
        ) as mock_recognition:
    ...
    async def test_text_to_speech_full_flow(self, voice_service):
        ...
        with patch.object(voice_service, "_generate_speech") as mock_generation:
            with patch.object(
                voice_service, "_calculate_audio_duration"
            ) as mock_duration:
                mock_generation.return_value = b"fake_audio_data"
                mock_duration.return_value = 2.5

                result = await voice_service.text_to_speech(
                    text="Hello world", session_id="test-session", user_id="test-user"
                )
    ...
    async def test_cache_functionality(self, voice_service, sample_audio_data):
        ...
        with patch.object(
            ...
            mock_recognition.return_value = {"text": "Cached text", "confidence": 0.8}
...
class TestVoiceAPI:
    ...
    def mock_user(self):
        """Mock user fixture"""
        return {"user_id": "test-user", "username": "testuser", "is_admin": False}
