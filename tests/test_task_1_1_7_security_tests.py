...
class SecurityTestHelper:
    ...
    def test_payload_injection(
        self, endpoint: str, method: str, payloads: list[str], field_name: str = "email"
    ) -> dict:
        ...
        # # response = self.client.post(endpoint, json=data)  # noqa: F841  # noqa: F841
                ...
                if response.status_code in [400, 422, 500]:
                    ...
                    if any(
                        danger in response_text
                        for danger in ["<script", "javascript:", "onerror=", "onload="]
                    ):
    ...
    def cleanup_test_data(self):
        ...
        try:
            db = SessionLocal()
            test_users = (
                db.query(User).filter(User.email.like("security_test_user_%")).all()
            )
...
class TestInputValidationSecurity:
    ...
    def test_xss_protection_email_validation(self):
        ...
        assert (
            results["safe_responses"] + results["error_responses"]
            >= len(XSS_PAYLOADS) * 0.8
        )
    ...
    def test_sql_injection_protection(self):
        ...
        print(
            f"✅ Safe responses: {results['safe_responses']}/{len(SQL_INJECTION_PAYLOADS)}"
        )
        print(f"❌ Vulnerable responses: {results['vulnerable_responses']}")

        # SQL injection should be blocked
        assert (
            results["vulnerable_responses"] == 0
        ), "SQL injection vulnerabilities detected!"
        assert (
            results["safe_responses"] + results["error_responses"]
            >= len(SQL_INJECTION_PAYLOADS) * 0.8
        )
    ...
    def test_command_injection_protection(self):
        ...
        for endpoint, method, field in test_endpoints:
            ...
            print(
                f"✅ {endpoint}: {results['safe_responses']} safe, {results['vulnerable_responses']} vulnerable"
            )
    ...
    def test_oversized_input_protection(self):
        ...
        for payload in OVERSIZED_PAYLOADS[:2]:  # Test first 2 to avoid timeouts
            try:
                ...
                if response.status_code in [
                    ...
                    safe_responses += 1
                    print(
                        f"✅ Rejected oversized input ({len(payload)} chars): {response.status_code}"
                    )
                else:
                    print(
                        f"⚠️ Accepted oversized input ({len(payload)} chars): {response.status_code}"
                    )
        ...
        assert (
            safe_responses >= 1
        ), "System should reject at least some oversized inputs"
    ...
    def test_special_characters_handling(self):
        ...
        for payload in special_payloads:
            try:
                ...
                print(
                    f"✅ Special char test: {response.status_code} for {payload[:30]}..."
                )
        ...
        assert (
            safe_count >= len(special_payloads) * 0.8
        )  # Most should be handled safely
    ...
    def test_password_security_requirements(self):
        ...
        # # response = security_helper.client.post(  # noqa: F841  # noqa: F841
            ...
            if response.status_code in [400, 422]:
                rejected_count += 1
                print(
                    f"✅ Rejected weak password: '{weak_password}' -> {response.status_code}"
                )
            else:
                print(
                    f"⚠️ Accepted weak password: '{weak_password}' -> {response.status_code}"
                )

        # At least half of weak passwords should be rejected
        assert (
            rejected_count >= len(weak_passwords) * 0.5
        ), "Too many weak passwords accepted"
...
class TestSecurityHeaders:
    ...
    def test_security_headers_presence(self):
        ...
        for endpoint in test_endpoints:
            ...
            ], f"Content-Type missing for {endpoint}"
    ...
    def test_error_information_disclosure(self):
        ...
        # # response = security_helper.client.post(endpoint, json=data)  # noqa: F841  # noqa: F841
                ...
                sensitive_found = any(
                    re.search(pattern, response_text) for pattern in sensitive_patterns
                )
                ...
                if response.status_code >= 500:
                    assert (
                        not sensitive_found
                    ), f"Sensitive information disclosed in server error for {endpoint}"
        ...
        # # response = security_helper.client.post(endpoint, json=data)  # noqa: F841  # noqa: F841
                ...
                if "application/json" in content_type:
                    ...
                    if (
                        ...
                        and "&lt;script&gt;" not in html_content
                    ):
    ...
    for test_class in test_classes:
        try:
            exit_code = pytest.main(
                [f"{__file__}::{test_class}", "-v", "-s", "--tb=short", "--maxfail=5"]
            )
