...
class TestCapeAIEndpoints:
    ...
    def test_ai_prompt_with_conversation_history(self, mock_redis, mock_openai):
        ...
        mock_openai.return_value.choices = [
            MagicMock(
                message=MagicMock(
                    content="Based on our previous conversation about CapeControl, I can provide more details."
                )
            )
        ]
    ...
    def test_ai_prompt_openai_error_fallback(self, mock_redis, mock_openai):
        ...
        assert (
            "help" in data["response"].lower() or "navigate" in data["response"].lower()
        )
    ...
    def test_get_conversation_history_success(self, mock_redis):
        ...
        response = client.get(
            f"/api/ai/conversation/{self.session_id}", headers=self.headers
        )
    ...
    def test_clear_conversation_success(self, mock_redis):
        ...
        assert (
            "cleared" in data["message"].lower()
            or "conversation" in data["message"].lower()
        )

    def test_get_contextual_suggestions_dashboard(self):
        """Test contextual suggestions for dashboard page"""
        response = client.get(
            "/api/ai/suggestions?current_path=/dashboard", headers=self.headers
        )
        ...
        assert any(
            ...
            for word in ["analytics", "metrics", "performance", "dashboard"]
        )

    def test_get_contextual_suggestions_agents_page(self):
        """Test contextual suggestions for agents page"""
        response = client.get(
            "/api/ai/suggestions?current_path=/agents", headers=self.headers
        )
...
class TestCapeAIService:
    ...
    def setup_method(self):
        ...
        self.test_user.created_at = datetime.now() - timedelta(
            days=10
        )  # 10 days old account
    ...
    def test_get_platform_context_dashboard(self):
        ...
        assert (
            "dashboard_navigation" in context["help_topics"]
            or "key_metrics" in context["help_topics"]
        )
    ...
    def test_get_platform_context_agents(self):
        ...
        assert "primary_actions" in context
        assert (
            "agent_selection" in context["help_topics"]
            or "agent_configuration" in context["help_topics"]
        )
    ...
    def test_build_system_prompt(self):
        ...
        user_context = {
            ...
            "platform_context": {
                "area": "dashboard",
                "help_topics": ["dashboard_navigation"],
            },
        }
    ...
    def test_generate_suggestions_dashboard_context(self):
        ...
        assert any(
            word in suggestions_text
            for word in ["analytics", "usage", "agents", "dashboard"]
        )

    def test_generate_actions_dashboard_context(self):
        """Test action generation for dashboard context"""
        context = {
            "platform_context": {"primary_actions": ["view_analytics", "manage_agents"]}
        }
...
class TestCapeAIIntegration:
    ...
    def test_complete_conversation_workflow(self, mock_redis, mock_openai):
        ...
        mock_openai.return_value.choices = [
            MagicMock(
                message=MagicMock(
                    content="Hello! I'm CapeAI. How can I help you today?"
                )
            )
        ]
        ...
        response = client.get(
            f"/api/ai/conversation/{session_id}", headers=self.headers
        )
        assert response.status_code == 200
