...
class TestMultiProviderAIService:
    ...
    def mock_performance_monitor(self):
        ...
        with patch(
            "app.services.multi_provider_ai_service.get_ai_performance_monitor"
        ) as mock_monitor:
            monitor = Mock()
    ...
    def service(self, mock_settings, mock_performance_monitor):
        ...
        with patch(
            "app.services.multi_provider_ai_service.AsyncOpenAI"
    ...
    async def test_openai_response_generation(self, service):
        ...
        response = await service.generate_response(
            messages, model="gpt-4", user_id="test-user"
        )
    ...
    async def test_claude_response_generation(self, service):
        ...
        # Configure mock client
        service.clients[ModelProvider.CLAUDE].messages.create = AsyncMock(
            return_value=mock_response
        )
        ...
        # Generate response
        response = await service.generate_response(
            messages, model="claude-3-sonnet", user_id="test-user"
        )
    ...
    async def test_performance_monitoring_integration(
        self, service, mock_performance_monitor
    ):
...
class TestGlobalServiceInstance:
    ...
    def test_singleton_pattern(self):
        ...
        with patch(
            "app.services.multi_provider_ai_service.MultiProviderAIService"
        ) as MockService:
            mock_instance = Mock()
...
class TestModelConfigurations:
    ...
    def test_openai_model_configs(self):
        ...
        with patch("app.services.multi_provider_ai_service.settings") as mock_settings:
            ...
            with patch("app.services.multi_provider_ai_service.AsyncOpenAI"), patch(
                "app.services.multi_provider_ai_service.anthropic.AsyncAnthropic"
            ), patch(
                "app.services.multi_provider_ai_service.get_ai_performance_monitor"
            ):
...
class TestMultiProviderIntegration:
    ...
    async def test_end_to_end_openai_flow(self):
        ...
        with patch(
            "app.services.multi_provider_ai_service.settings"
    ...
    async def test_end_to_end_claude_flow(self):
        ...
        with patch(
            ...
            response = await service.generate_response(
                messages, model="claude-3-sonnet"
            )
