        ...
        with engine.begin() as conn:
            ...
            if "audit_logs" in tables:
                ...
                for fk in fks:
                    if fk["referred_table"] == "users":
                        ...
                        try:
                            conn.execute(
                                text(
                                    f"ALTER TABLE audit_logs DROP CONSTRAINT IF EXISTS {fk['name']}"
                                )
                            )
                ...
                try:
                    ...
                    print(
                        "✅ Added foreign key constraint: audit_logs.user_id -> users_v2.id"
                    )
                except Exception as e:
                    print(
                        f"⚠️ Foreign key constraint might already exist or error occurred: {e}"
                    )
            ...
            if "conversations" in tables:
                ...
                for fk in fks:
                    if fk["referred_table"] == "users":
                        print(
                            f"🔧 Dropping incorrect conversation foreign key: {fk['name']}"
                        )
                        try:
                            conn.execute(
                                text(
                                    f"ALTER TABLE conversations DROP CONSTRAINT IF EXISTS {fk['name']}"
                                )
                            )
                        except Exception as e:
                            print(f"⚠️ Could not drop constraint {fk['name']}: {e}")
                ...
                try:
                    ...
                    print(
                        "✅ Added foreign key constraint: conversations.user_id -> users_v2.id"
                    )
                except Exception as e:
                    print(
                        f"⚠️ Conversations foreign key constraint might already exist: {e}"
                    )
...
def validate_relationships():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ AuditLog-User relationship working: {len(recent_logs)} logs retrieved"
            )
        except Exception as e:
