...
# Module-level fixture so multiple test classes (including performance) can share it
@pytest.fixture
def mock_audit_service():  # type: ignore
    service = AIAuditService(redis_url="redis://invalid:0")
...
class TestAIAuditService:
    ...
    @pytest.fixture
    def mock_audit_service(self):  # type: ignore
        """Synchronous fixture returning initialized service using a temporary loop."""
        loop = asyncio.new_event_loop()
        try:

            async def _create():
                service = AIAuditService(redis_url="redis://invalid:0")
                # Force memory mode (skip redis & db persistence)
                service.redis_client = None
                service.db_session = None
                # Avoid calling initialize which would attempt redis connection
                # Instead mimic post-initialize state.
                service.memory_cache = {}
                return service
    ...
    async def test_log_ai_interaction_with_metadata(self, mock_audit_service):
        ...
        metadata = {"temperature": 0.7, "max_tokens": 100, "custom_field": "test_value"}
    ...
    async def test_error_handling(self, mock_audit_service):
        ...
        audit_id = await mock_audit_service.log_ai_interaction(
            user_id="", provider="openai", model="gpt-4", prompt="Test", response="Test"
        )
        assert audit_id is not None
...
class TestAuditServicePerformance:
    ...
    async def test_logging_performance(self, mock_audit_service):
        ...
        ), f"Logging too slow: {avg_time_per_log:.4f}s per log"
...
if __name__ == "__main__":
    ...
    async def run_basic_tests():
        ...
        print(
            f"✅ Generated compliance report with {len(report.get('recommendations', []))} recommendations"
        )
