...
try:
    ...
    version = getattr(stripe, "__version__", None)
    if not version:
        # Older tests expect stripe.version.VERSION
        version_mod = getattr(stripe, "version", None)
        version = (
            getattr(version_mod, "VERSION", "unknown") if version_mod else "unknown"
        )
