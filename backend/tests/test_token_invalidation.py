...
def test_token_invalidated_after_password_change():
    ...
    # Old token should now be rejected. There can be tight timing races between
    # the password_changed_at update and token validation, so poll briefly.
    r2 = None
    for _ in range(5):
        r2 = get_me(access)
        if r2.status_code == 401:
            break
    assert r2 is not None and r2.status_code == 401
