...
class TestUserProfileModel:
    ...
    def test_user_profile_json_data(self, db: Session):
        ...
        profile = UserProfile(user_id=2, profile_data=json.dumps(profile_data))
...
class TestDeveloperEarningModel:
    ...
    def test_developer_earning_defaults(self, db: Session):
        ...
        earning = DeveloperEarning(user_id=user.id, agent_id="default_agent")
    ...
    def test_developer_earning_payout_tracking(self, db: Session):
        ...
        assert earning.last_payout_at == payout_time.replace(tzinfo=None)
    ...
    def test_developer_earning_user_relationship(self, db: Session):
        ...
        earning = DeveloperEarning(user_id=user.id, agent_id="rel_agent")
        db.add(earning)
        db.commit()
...
class TestPasswordResetModel:
    ...
    def test_password_reset_creation(self, db: Session):
        ...
        assert reset.expires_at > utc_now().replace(tzinfo=None)
...
class TestModelIntegration:
    ...
    def test_json_field_serialization(self, db: Session):
        ...
        reloaded_user = (
            db.query(UserV2).filter(UserV2.email == "json@example.com").first()
        )
...
class TestModelPerformance:
    ...
    def test_large_json_fields(self, db: Session):
        ...
        reloaded_user = (
            db.query(UserV2).filter(UserV2.email == "largejson@example.com").first()
        )
