...
class UserProfile(BaseModel):
    ...
    communication_style: str = Field(
        default="balanced", description="User's communication style"
    )
    learning_style: str = Field(default="mixed", description="User's learning style")
    expertise_level: str = Field(
        default="intermediate", description="User's expertise level"
    )
...
class ContextAwareRequest(BaseModel):
    ...
    preferred_strategy: str | None = Field(
        default=None, description="Preferred response strategy"
    )
...
class PerformanceMetrics(BaseModel):
    ...
    average_context_quality: str = Field(
...
async def analyze_conversation_context(
    ...
    try:
        ...
        analysis_result = await context_aware_ai_service.get_context_analysis(
            user_id=request.user_id, conversation_history=conversation_history_dict
        )
...
async def get_response_strategies() -> list[dict[str, str]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get response strategies: {str(e)}"
        )
...
async def get_context_types() -> list[dict[str, str]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get context types: {str(e)}"
        )
...
async def generate_quick_response(
    ...
    strategy: str | None = Field(
        default=None, description="Optional response strategy"
    ),
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate quick response: {str(e)}"
        )
...
async def batch_analyze_conversations(
    ...
    try:
        if len(requests) > 50:  # Limit batch size
            raise HTTPException(
                status_code=400, detail="Batch size cannot exceed 50 conversations"
            )
        ...
        for request in requests:
            try:
                ...
                results.append(
                    {"user_id": request.user_id, "error": str(e), "status": "error"}
                )
...
async def simulate_conversation_flow(
    ...
    max_queries: int = Field(
        default=10, description="Maximum number of queries to process"
    ),
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        for i, query in enumerate(queries):
            ...
            user_msg = ChatMessage(
                role="user", content=query, timestamp=utc_now().isoformat()
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to simulate conversation: {str(e)}"
        )
