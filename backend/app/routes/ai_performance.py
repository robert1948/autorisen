...
async def get_ai_performance_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get AI performance status: {str(e)}"
        )
...
async def get_real_time_ai_metrics(
    ...
    try:
        ...
        return {"real_time_metrics": metrics, "timestamp": utc_now().isoformat()}
...
async def get_ai_performance_statistics(
    ...
    time_period: str = Query("1h", pattern="^(\\d+[mhd]|1h|24h|7d)$"),
    ...
    try:
        ...
        stats = monitor.get_performance_stats(
            provider=provider, model=model, time_period=time_period
        )
...
async def get_ai_cost_analytics(
    time_period: str = Query("24h", pattern="^(\\d+[mhd]|1h|24h|7d|30d)$"),
...
async def get_ai_usage_patterns(
    time_period: str = Query("24h", pattern="^(\\d+[mhd]|1h|24h|7d)$"),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get usage patterns: {str(e)}"
        )
...
async def get_ai_services_health(
    ...
    try:
        ...
        return {"health_status": health_status, "timestamp": utc_now().isoformat()}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get AI health status: {str(e)}"
        )


@router.get(
    "/optimization/recommendations", summary="Get AI optimization recommendations"
)
...
async def record_ai_usage_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to record AI metrics: {str(e)}"
        )
...
async def get_supported_ai_providers(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get provider information: {str(e)}"
        )
...
async def get_ai_models_for_provider(
    ...
    try:
            ...
            models_info.append(
                {
                    ...
                    "completion_cost_per_1k": cost_config.get(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get models for provider: {str(e)}"
        )
...
async def get_ai_analytics_summary(
    ...
    try:
        ...
        total_requests = sum(
            stats.total_requests for stats in performance_stats.values()
        )
        ...
        return {
            "summary": {
                ...
                "active_providers": len(
                    set(stats.provider.value for stats in performance_stats.values())
                ),
