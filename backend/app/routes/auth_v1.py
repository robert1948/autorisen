...
async def validate_password(password_data: dict):
    ...
    return {
        ...
        "message": (
            "Strong password" if all_valid else "Password doesn't meet requirements"
        ),
    }
...
async def login_v2(
    ...
    try:
        ...
        audit_logger.log_authentication_event(
            ...
            user_role=user.user_role,
            success=True,
            metadata={"login_method": "v2_password", "user_role": user.user_role},
        )
...
async def register_step1(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Step 1 validation failed: {str(e)}"
        )
...
async def register_step2(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Password hashing failed: {str(hash_error)}"
            )
        ...
        db_user = models.User(
            ...
            terms_accepted_at=utc_now(),  # Use 'terms_accepted_at' column name
        )
        ...
        print(
            f"✅ Step2 Debug: Registration completed successfully for '{normalized_email}'"
        )
