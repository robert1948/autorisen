...
async def get_analytics_dashboard(
    ...
    try:
        ...
        if current_user.role not in ["ADMIN", "DEVELOPER"]:
            raise HTTPException(
                status_code=403,
                detail="Analytics access requires admin or developer role",
            )
...
async def get_realtime_metrics(
    ...
    try:
        ...
        return {"status": "success", "data": metrics, "user_role": current_user.role}
...
async def get_user_activity(
    ...
    try:
        if current_user.role != "ADMIN":
            raise HTTPException(
                status_code=403, detail="User activity analytics requires admin role"
            )
        ...
        # Unique users
        unique_users = (
            base_query.filter(AnalyticsEvent.user_id.isnot(None))
        ...
        # Recent events
        recent_events = (
            base_query.order_by(desc(AnalyticsEvent.created_at)).limit(50).all()
        )
