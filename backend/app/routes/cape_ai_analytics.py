...
import logging
from datetime import datetime, timedelta  # legacy naive
...
async def get_user_quality_scores(
    ...
    try:
        logger.info(
            f"Fetching quality scores for user {user_id}, timeframe: {days} days"
        )
        ...
        logger.info(
            f"Quality scores calculated for user {user_id}: overall {quality_data['overall']}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving quality scores: {str(e)}"
        )
...
async def get_provider_breakdown(
    ...
    try:
        logger.info(
            f"Fetching provider breakdown for user {user_id}, timeframe: {days} days"
        )
        ...
        for provider, stats in provider_stats.items():
            if stats["interactions"] > 0:
                ...
                primary_model = (
                    max(stats["models"].items(), key=lambda x: x[1]["interactions"])[0]
                    if stats["models"]
                ...
                provider_data = {
                    ...
                    "avg_tokens": (
                        stats["tokens_used"] / stats["interactions"]
                        if stats["interactions"] > 0
                        else 0
                    ),
        ...
        summary = {
            ...
            "avg_tokens_per_interaction": (
                round(total_tokens / total_interactions, 1)
                if total_interactions > 0
                else 0
            ),
            ...
            "cost_per_interaction": (
                round(total_cost / total_interactions, 3)
                if total_interactions > 0
                else 0
            ),
        }
        ...
        logger.info(
            f"Provider breakdown calculated for user {user_id}: {len(provider_breakdown)} providers"
        )
        ...
        logger.error(
            f"Error retrieving provider breakdown for user {user_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail=f"Error retrieving provider breakdown: {str(e)}"
        )
...
async def get_system_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving system performance: {str(e)}"
        )
...
async def get_interaction_history(
    ...
    provider: str | None = Query(
        None, description="Filter by provider (openai, claude, gemini)"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "filters": {"days": days, "limit": limit, "provider": provider},
            },
            "timestamp": utc_now().isoformat(),
            "user_id": user_id,
        }
        ...
        logger.error(
            f"Error retrieving interaction history for user {user_id}: {str(e)}"
        )
        raise HTTPException(
...
async def general_exception_handler(request, exc):
    ...
    return HTTPException(
        status_code=500, detail="Internal server error in analytics service"
    )
