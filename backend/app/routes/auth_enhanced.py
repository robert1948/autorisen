...
from datetime import datetime  # retained for legacy values; prefer utc_now
...
async def debug_database_test(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"status": "error", "error": str(e), "database_connected": False}
...
def logout(
    ...
    try:
        ...
        # Also revoke any refresh tokens for this user session
        # In production, you might want to track refresh tokens per session
        # For now, we'll revoke all refresh tokens for the user
...
async def request_password_reset(
    ...
    try:
        ...
        return PasswordResetResponse(
            message="If the email exists, a reset link has been sent", email_sent=True
        )
...
def get_developer_earnings(
    ...
    try:
        ...
        earnings = (
            db.query(DeveloperEarning)
            .filter(DeveloperEarning.user_id == current_user.id)
            .all()
        )
