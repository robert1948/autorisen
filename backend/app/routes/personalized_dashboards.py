...
class WidgetConfigModel(BaseModel):
    ...
    is_resizable: bool = Field(
        default=True, description="Whether widget can be resized"
    )
...
class UserDashboardModel(BaseModel):
    ...
    personalization_level: float = Field(
        ..., description="Personalization level (0.0-1.0)"
    )
    created_at: str | None = Field(default=None, description="Creation timestamp")
...
class CreateDashboardRequest(BaseModel):
    ...
    usage_history: list[dict[str, Any]] = Field(
        default_factory=list, description="User usage history"
    )
    role_override: str | None = Field(
        default=None, description="Override automatic role detection"
    )
...
class WidgetInteractionRequest(BaseModel):
    ...
    interaction_type: str = Field(
        ..., description="Interaction type (click, resize, move, etc.)"
    )
    duration_seconds: float | None = Field(
        default=None, description="Interaction duration"
    )
...
class DashboardAnalyticsResponse(BaseModel):
    ...
    personalization_metrics: dict[str, Any] = Field(
        ..., description="Personalization metrics"
    )
...
async def create_personalized_dashboard(
    ...
    try:
        ...
        dashboard = await dashboard_manager.create_dashboard(
            user_profile=request.user_profile, usage_history=request.usage_history
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create personalized dashboard: {str(e)}"
        )
...
async def update_dashboard(
    ...
    try:
        updated_dashboard = await dashboard_manager.update_dashboard(
            dashboard_id=dashboard_id, updates=request.updates
        )
...
async def adapt_dashboard(
    dashboard_id: str, request: AdaptDashboardRequest
) -> UserDashboardModel:
    """
    ...
    try:
        adapted_dashboard = await dashboard_manager.adapt_dashboard(
            dashboard_id=dashboard_id, interaction_data=request.interaction_data
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to adapt dashboard: {str(e)}"
        )


@router.post("/{dashboard_id}/track-interaction")
async def track_widget_interaction(
    dashboard_id: str, interaction: WidgetInteractionRequest
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        adapted_dashboard = await dashboard_manager.adapt_dashboard(
            dashboard_id=dashboard_id, interaction_data=interaction_data
        )
...
async def import_dashboard(request: ImportDashboardRequest) -> dict[str, str]:
    ...
    try:
        ...
        if not dashboard_id:
            raise HTTPException(
                status_code=400, detail="Invalid dashboard data or import failed"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to import dashboard: {str(e)}"
        )
...
async def get_available_roles() -> list[dict[str, str]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get available roles: {str(e)}"
        )
...
async def get_available_widget_types() -> list[dict[str, str]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get available widget types: {str(e)}"
        )
...
async def get_available_layout_types() -> list[dict[str, str]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get available layout types: {str(e)}"
        )


@router.post("/{dashboard_id}/widgets", response_model=dict[str, Any])
async def add_widget_to_dashboard(
    dashboard_id: str, widget_config: WidgetConfigModel
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        active_layout = next(
            (
                ...
                if layout.layout_id == dashboard.active_layout
            ),
            dashboard.layouts[0] if dashboard.layouts else None,
        )

        if not active_layout:
            raise HTTPException(status_code=400, detail="No active layout found")

        active_layout.widgets.append(new_widget)
        active_layout.updated_at = utc_now().isoformat()

        # Update dashboard
        await dashboard_manager.update_dashboard(
            dashboard_id, {"layouts": dashboard.layouts}
        )
...
async def remove_widget_from_dashboard(
    dashboard_id: str, widget_id: str
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        active_layout.widgets = [
            widget for widget in active_layout.widgets if widget.widget_id != widget_id
        ]

        if len(active_layout.widgets) == original_count:
            raise HTTPException(
                status_code=404, detail=f"Widget {widget_id} not found in dashboard"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to remove widget: {str(e)}"
        )
