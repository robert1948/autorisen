...
async def faq_agent(request: FAQRequest, db: Session = Depends(get_db)) -> FAQResponse:
    ...
    try:
        ...
        user_msg.created_at = utc_now()
        db.add(user_msg)
        db.commit()
        db.refresh(user_msg)

        # Generate demo AI response via cape_ai_service
        ai_result = await cape.generate_response(
            request.question, context={"path": "/agents/faq"}
        )
        answer_text = (
        ...
        assistant_msg.created_at = utc_now()
        ...
        # isoformat() on aware dt includes +00:00; test only asserts presence of timestamp
        return FAQResponse(
            conversation_id=conv_id, answer=answer_text, timestamp=utc_now().isoformat()
        )
