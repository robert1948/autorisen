...
import asyncio
import json
from datetime import datetime  # legacy naive usage
...
async def get_alert_system_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get alert system status: {str(e)}"
        )
...
async def get_active_alerts(
    ...
    try:
        ...
        alerts = alert_system.get_active_alerts(
            severity=severity, alert_type=alert_type, tags=tag_list
        )
...
async def acknowledge_alert(
    ...
    try:
        ...
        success = await alert_system.acknowledge_alert(
            alert_id, acknowledge_data.acknowledged_by
        )
...
async def create_manual_alert(
    ...
    try:
        ...
        if not alert_id:
            raise HTTPException(
                status_code=400,
                detail="Failed to create alert (rule not found or in cooldown)",
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create manual alert: {str(e)}"
        )
...
async def get_alert_rules(
    ...
    try:
        ...
        return {"rules": rules, "count": len(rules), "timestamp": utc_now().isoformat()}
...
async def create_notification_channel(
    ...
    try:
        ...
        if channel_data.name in alert_system.notification_channels:
            raise HTTPException(
                status_code=400,
                detail="Notification channel with this name already exists",
            )
        ...
        safe_config = {
            k: v
            for k, v in channel_data.config.items()
            if "password" not in k.lower() and "token" not in k.lower()
        }
        audit_logger = get_audit_logger()
        audit_logger.log_system_event(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create notification channel: {str(e)}"
        )
...
async def get_alert_statistics(
    ...
    try:
        ...
        return {"statistics": stats, "timestamp": utc_now().isoformat()}
...
async def test_notification_channel(
    ...
    try:
        ...
        test_alert = Alert(
            id=f"test_{int(utc_now().timestamp())}",
            rule_name="test_rule",
