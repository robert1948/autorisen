...
import logging
from datetime import datetime, timedelta  # legacy naive now
...
async def track_bulk_events(
    ...
    try:
        if not bulk_request.events:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No events provided"
            )
...
async def get_detailed_analytics(
    ...
    group_by: str = Query(
        "day", description="Group results by: hour, day, week, month"
    ),
    ...
    try:
        ...
        logger.error(
            f"Error getting detailed analytics for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_performance_metrics(
    ...
    try:
        ...
        logger.error(
            f"Error getting performance metrics for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_analytics_data(
    ...
    days_to_keep: int = Query(
        0, ge=0, le=365, description="Days of data to keep (0 = delete all)"
    ),
    current_user: User = Depends(get_current_user),
    analytics_service=Depends(get_analytics_service),
):
...
@router.get("/health")
async def analytics_service_health(analytics_service=Depends(get_analytics_service)):
...
async def process_real_time_analytics(user_id: str, event_type: str, event_name: str):
    ...
    try:
        logger.info(
            f"Processing real-time analytics for user {user_id}: {event_type}.{event_name}"
        )
...
async def process_bulk_analytics(
    user_id: str, successful_tracks: int, total_events: int
):
    """Process bulk analytics for batch insights"""
    try:
        logger.info(
            f"Processed bulk analytics for user {user_id}: {successful_tracks}/{total_events} events"
        )
