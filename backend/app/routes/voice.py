...
async def get_voice_config(service: VoiceService = Depends(get_voice_service)):
...
async def speech_to_text(
    ...
    try:
        service: VoiceService = await get_voice_service()
        # Decode audio data
        try:
...
@router.get(
    "/analytics",
    response_model=Union[VoiceAnalyticsResponse, dict[str, VoiceAnalyticsResponse]],
)
    ...
    try:
        ...
        if session_id:
            ...
            return VoiceAnalyticsResponse(
                ...
                preferred_provider=(
                    analytics.preferred_provider.value
                    if analytics.preferred_provider
                    else None
                ),
        ...
        return {
            sid: VoiceAnalyticsResponse(
                ...
                preferred_provider=(
                    data.preferred_provider.value if data.preferred_provider else None
                ),
...
@router.get("/health")
async def voice_health_check(service: VoiceService = Depends(get_voice_service)):
...
async def voice_echo_test(
    ...
    try:
        ...
        return {
            ...
            "total_processing_time": stt_result.processing_time
...
async def voice_websocket(
    websocket: WebSocket, service: VoiceService = Depends(get_voice_service)
):
    """WebSocket endpoint for real-time voice interaction"""
    ...
    try:
        while True:
            ...
            try:
                ...
                stt_result = await service.speech_to_text(
                    audio_data=data, audio_format=AudioFormat.WAV, language="en-US"
                )
