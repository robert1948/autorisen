...
class ConversationService:
    ...
    async def create_conversation(
        ...
        try:
            ...
            conversation_data = {
                "title": title
                or f"Conversation {utc_now().strftime('%Y-%m-%d %H:%M')}",
    ...
    async def get_conversation(
        ...
        try:
            ...
            if conversation.user_id != user_id:
                logger.warning(
                    f"Unauthorized access to conversation {conversation_id} by user {user_id}"
                )
                return None
            ...
            return {
                ...
                "tags": conversation.tags,
                "threads": [
                    thread.to_dict() for thread in conversation.threads.values()
                ],
            }
    ...
    async def add_message(
        ...
        try:
            ...
            if not conversation or conversation.user_id != user_id:
                logger.warning(
                    f"Unauthorized or invalid conversation access: {conversation_id}"
                )
                return None
            ...
            message_obj = await self.conversation_manager.add_message(
                conversation_id, message_role, message, {"user_id": user_id}
            )
    ...
    async def process_ai_message(
        ...
        try:
            ...
            user_msg = await self.add_message(
                db, conversation_id, user_id, user_message, "user"
            )
            if not user_msg:
                return None
            ...
            ai_context, context_metadata = (
                await context_service.generate_context_for_ai(
                    conversation_id, max_context_messages=10, include_summary=True
                )
            )
            ...
            ai_response = await self._generate_ai_response_with_context(
                user_message, full_context, ai_provider
            )
            ...
            if ai_response:
                ...
                ai_msg = await self.add_message(
                    db, conversation_id, user_id, ai_response["content"], "assistant"
                )
                ...
                if ai_msg:
                    ...
                    logger.info(
                        f"AI response generated for conversation {conversation_id}"
                    )
                    return ai_msg
    ...
    async def _generate_ai_response_with_context(
        self, message: str, context: dict[str, Any], provider: str
    ) -> dict[str, Any] | None:
