...
class AuditLogger:
    ...
    def log_event(
        ...
        **kwargs,
        ...
        try:
            ...
            log_message = (
                f"AUDIT: {event_type.value} - User: {user_email or 'Anonymous'} - "
    ...
    def log_ai_event(
        ...
        metadata = {
            "ai_service": True,
            "prompt_length": (
                len(str(prompt_data.get("message", ""))) if prompt_data else 0
            ),
            "response_length": (
                len(str(response_data.get("response", ""))) if response_data else 0
            ),
            "moderation_applied": moderation_applied,
            "processing_time_ms": processing_time_ms,
            "model_used": (
                response_data.get("model", "unknown") if response_data else None
            ),
        }
    ...
    def get_user_activity(
        ...
        query = db.query(AuditLog).filter(
            and_(AuditLog.user_id == user_id, AuditLog.created_at >= cutoff_date)
        )
    ...
    def get_security_events(
        self, db: Session, hours: int = 24, severity_level: AuditLogLevel | None = None
    ) -> list[AuditLog]:
        """
        Get recent security events for monitoring
        """
        cutoff_date = utc_now() - timedelta(hours=hours)
    ...
    def get_failed_authentication_attempts(
        self, db: Session, hours: int = 1, ip_address: str | None = None
    ) -> list[AuditLog]:
        """
    ...
    def get_audit_statistics(self, db: Session, days: int = 7) -> dict[str, Any]:
        ...
        # Events by category
        category_stats = (
            db.query(AuditLog.event_category, func.count(AuditLog.id).label("count"))
            .filter(AuditLog.created_at >= cutoff_date)
        ...
        failed_events = (
            db.query(func.count(AuditLog.id))
            .filter(and_(AuditLog.created_at >= cutoff_date, AuditLog.success == False))
            .scalar()
        )
        ...
        high_risk_events = (
            db.query(func.count(AuditLog.id))
            .filter(and_(AuditLog.created_at >= cutoff_date, AuditLog.risk_score >= 70))
            .scalar()
        )

        # Top users by activity
        top_users = (
            db.query(AuditLog.user_email, func.count(AuditLog.id).label("event_count"))
            .filter(
                and_(
                    AuditLog.created_at >= cutoff_date, AuditLog.user_email.isnot(None)
                )
            )
        ...
        return {
            ...
            "success_rate": round(
                (total_events - failed_events) / max(total_events, 1) * 100, 2
            ),
            "events_by_category": {cat: count for cat, count in category_stats},
            "top_users": [
                {"email": email, "event_count": count} for email, count in top_users
            ],
        }
