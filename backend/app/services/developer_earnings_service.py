...
class DeveloperEarningsService:
    ...
    async def get_developer_earnings_summary(
        self, developer_id: str, db: Session
    ) -> dict[str, Any]:
        ...
        try:
            ...
            earnings_data = {
                "developer_info": {
                    ...
                    or developer.email,
                    "joined_date": (
                        developer.created_at.isoformat()
                        if developer.created_at
                        else None
                    ),
                },
                "total_earnings": await self._calculate_total_earnings(
                    developer_id, db
                ),
                "today_earnings": await self._calculate_period_earnings(
                    developer_id, today, now, db
                ),
                "week_earnings": await self._calculate_period_earnings(
                    developer_id, week_start, now, db
                ),
                "month_earnings": await self._calculate_period_earnings(
                    developer_id, month_start, now, db
                ),
                "year_earnings": await self._calculate_period_earnings(
                    developer_id, year_start, now, db
                ),
                "recent_transactions": await self._get_recent_transactions(
                    developer_id, db
                ),
                "earnings_chart_data": await self._get_earnings_chart_data(
                    developer_id, db
                ),
    ...
    async def _calculate_period_earnings(
        self, developer_id: str, start_date: datetime, end_date: datetime, db: Session
    ) -> dict[str, Any]:
    ...
    async def _get_recent_transactions(
        self, developer_id: str, db: Session, limit: int = 10
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for i in range(min(limit, 5)):  # Generate up to 5 mock transactions
                ...
                mock_date = utc_now() - timedelta(days=i * 3)
    ...
    async def _get_earnings_chart_data(
        self, developer_id: str, db: Session
    ) -> dict[str, Any]:
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                base_daily_amount = 200 + (
                    hash(current_date.strftime("%Y-%m-%d")) % 800
                )  # $2-10 per day
    ...
    async def _get_payout_status(self, developer_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            next_payout = utc_now() + timedelta(days=7)
            ...
            return {
                ...
                "last_payout_date": (utc_now() - timedelta(days=7)).isoformat(),
    ...
    async def _get_mock_earnings_data(self, developer_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "total_earnings": {
                "total_earned": {
                    "amount_cents": 245000,
                    "amount_dollars": 2450.0,
                    "formatted": "$2,450.00",
                },
                "total_paid": {
                    "amount_cents": 220500,
                    "amount_dollars": 2205.0,
                    "formatted": "$2,205.00",
                },
                "pending_payout": {
                    "amount_cents": 24500,
                    "amount_dollars": 245.0,
                    "formatted": "$245.00",
                },
            },
            "today_earnings": {
