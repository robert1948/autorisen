...
class ContextAnalyzer:
    ...
    async def analyze_conversation_context(
        self, messages: list[dict[str, Any]], timeframe: timedelta = timedelta(hours=2)
    ) -> ContextWindow:
        """Analyze conversation context from recent messages"""
        try:
            # Filter messages by timeframe
            cutoff_time = utc_now() - timeframe
            recent_messages = [
                msg
                for msg in messages
                if datetime.fromisoformat(msg.get("timestamp", "2025-01-01"))
                > cutoff_time
            ]
    ...
    async def analyze_user_context(
        self, user_profile: dict[str, Any], conversation_history: list[dict[str, Any]]
    ) -> UserContext:
        ...
        try:
            ...
            preferred_response_length = (
                await self._determine_response_length_preference(conversation_history)
            )
            ...
            interaction_history = await self._build_interaction_history(
                conversation_history
            )
    ...
    async def _extract_topic_focus(self, messages: list[dict[str, Any]]) -> str:
        ...
        try:
            ...
            creative_score = sum(
                1 for keyword in creative_keywords if keyword in text_lower
            )
        ...
        try:
            ...
            return {
                ...
                "question_frequency": sum(
                    msg.get("content", "").count("?") for msg in user_messages
                )
                / max(len(user_messages), 1),
    ...
    async def _determine_response_length_preference(
        self, history: list[dict[str, Any]]
    ) -> str:
        ...
        try:
            ...
            avg_ai_length = sum(
                len(msg.get("content", "")) for msg in ai_messages
            ) / len(ai_messages)
            avg_user_length = sum(
                len(msg.get("content", "")) for msg in user_messages
            ) / len(user_messages)
    ...
    async def _analyze_current_emotional_state(
        ...
        try:
            ...
            recent_user_messages = [
                msg for msg in history[-5:] if msg.get("role") == "user"
            ]
    ...
    async def _build_interaction_history(
        self, history: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """Build interaction history summary"""
        try:
...
class ResponseGenerator:
    ...
    async def _select_response_strategy(
        ...
        try:
            ...
            if "creative" in user_style or topic_focus == "creative":
                ...
                context.user_context.learning_style == "visual"
                or "confused" in emotional_tone
            ):
                return ResponseStrategy.EDUCATIONAL
            elif (
                "excited" in emotional_tone
                or context.conversation_context.user_engagement > 0.7
            ):
    ...
    async def _generate_contextual_prompt(
        self, context: ResponseContext, strategy: ResponseStrategy
    ) -> str:
        """Generate a contextual prompt for the AI"""
        try:
            # Get strategy-specific prompt generator
            prompt_generator = self.strategy_prompts.get(
                strategy, self._create_adaptive_prompt
            )
    ...
    async def _create_context_enrichment(self, context: ResponseContext) -> str:
        ...
        try:
            ...
            if context.conversation_context.messages:
                ...
                if recent_topics:
                    enrichment_parts.append(
                        f"Recent conversation context: {list(recent_topics)}"
                    )

            # Add user interest context
            if context.user_context.interests:
                enrichment_parts.append(
                    f"User interests: {', '.join(context.user_context.interests)}"
                )
            ...
            return "Context Information:\n" + "\n".join(
                f"- {part}" for part in enrichment_parts
            )
...
class ContextAwareAIService:
    ...
    async def generate_response(
        ...
        try:
            ...
            conversation_context = (
                await self.context_analyzer.analyze_conversation_context(
                    conversation_history
                )
            )
            ...
            topic_context = {
                ...
                "topic_transitions": await self._analyze_topic_transitions(
                    conversation_history
                ),
            ...
            response_data = (
                await self.response_generator.generate_context_aware_response(
                    response_context
                )
            )
    ...
    async def get_context_analysis(
        self, user_id: str, conversation_history: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_processing_time_ms": self.performance_metrics[
                    "avg_processing_time"
                ]
                * 1000,
    ...
    def _calculate_session_duration(
        ...
        try:
            ...
            first_msg_time = datetime.fromisoformat(
                conversation_history[0].get("timestamp", "2025-01-01")
            )
            last_msg_time = datetime.fromisoformat(
                conversation_history[-1].get("timestamp", "2025-01-01")
            )
    ...
    async def _analyze_topic_transitions(
        self, conversation_history: list[dict[str, Any]]
    ) -> list[str]:
    ...
    def _update_performance_metrics(
        self, processing_time: float, context: ResponseContext
    ):
        ...
        try:
            ...
            self.performance_metrics["context_quality_scores"].append(
    ...
    async def _generate_quality_indicators(
        ...
        try:
            return {
                "context_richness": (
                    "high" if len(context.conversation_context.messages) > 5 else "low"
                ),
                "personalization_level": (
                    "high" if context.user_context.user_id else "none"
                ),
                "topic_clarity": (
                    "clear"
                    if context.conversation_context.topic_focus != "general"
                ...
                "user_engagement": (
                    "high"
                    if context.conversation_context.user_engagement > 0.7
                    else "moderate"
                ),
                "emotional_awareness": context.conversation_context.emotional_tone,
                "strategy_match": (
                    "optimal"
                    if context.strategy != ResponseStrategy.ADAPTIVE
                    else "default"
                ),
            }
        except Exception:
    ...
    async def _generate_follow_up_suggestions(
        ...
        try:
            ...
            if context.conversation_context.user_engagement > 0.8:
                suggestions.append(
                    "You seem very engaged! What aspect interests you most?"
                )
    ...
    async def _generate_context_recommendations(
        self, conv_context: ContextWindow, user_context: UserContext
    ) -> list[str]:
        ...
        try:
            ...
            if conv_context.relevance_score < 0.5:
                recommendations.append(
                    "Consider providing more specific information in your queries"
                )

            if user_context.communication_style == "balanced":
                recommendations.append(
                    "Your communication style could be further personalized"
                )

            if not user_context.interests:
                recommendations.append(
                    "Adding interests to your profile would improve personalization"
                )

            if conv_context.user_engagement < 0.5:
                recommendations.append(
                    "Try asking more specific questions to get better responses"
                )
    ...
    async def _calculate_context_quality(
        self, conv_context: ContextWindow, user_context: UserContext
    ) -> float:
        ...
        try:
            ...
            factors.append(
                (1.0 if user_context.communication_style != "balanced" else 0.5) * 0.1
            )
