...
from datetime import (
    datetime,
    timedelta,
)  # TODO: remove datetime after legacy timestamp parsing eliminated
...
class DashboardService:
    ...
    async def get_dashboard_data(self, time_window_hours: int = 24) -> DashboardData:
        ...
        health_data, performance_data, error_data, system_metrics = (
        ...
        self._cache_ttl[cache_key] = utc_now() + timedelta(seconds=self.cache_duration)
    ...
    async def get_real_time_dashboard(self) -> dict[str, Any]:
        ...
        real_time_data = {
            ...
            "system_health": {
                ...
                "memory_percent": health_data.get("system", {}).get(
                    "memory_percent", 0
                ),
                "disk_percent": health_data.get("system", {}).get("disk_usage", 0),
            },
            "alerts_count": len(health_data.get("issues", [])),
            "recent_activity": list(metrics_collector.recent_requests)[
                -5:
            ],  # Last 5 requests
        }
        # Cache with short TTL
        self._cache[cache_key] = real_time_data
        self._cache_ttl[cache_key] = utc_now() + timedelta(seconds=5)
    ...
    async def get_performance_charts(self, hours: int = 24) -> dict[str, Any]:
        ...
        charts_data = {
            ...
            "system_resources": {
                "cpu": self._aggregate_metrics_by_time(
                    cpu_metrics.get("system_cpu_percent", []), time_buckets
                ),
                "memory": self._aggregate_metrics_by_time(
                    memory_metrics.get("system_memory_percent", []), time_buckets
                ),
            },
            "request_volume": self._aggregate_metrics_by_time(
                request_metrics.get("http_requests_total", []), time_buckets
            ),
            "error_rate": self._aggregate_metrics_by_time(
                error_metrics.get("http_errors_total", []), time_buckets
            ),
    ...
    async def get_endpoint_analytics(self) -> dict[str, Any]:
        ...
        endpoints_by_volume = sorted(
            endpoint_stats.items(), key=lambda x: x[1].get("count", 0), reverse=True
        )[:10]
        ...
        endpoints_by_response_time = sorted(
            endpoint_stats.items(), key=lambda x: x[1].get("avg_time", 0), reverse=True
        )[:10]
        ...
        analytics = {
            ...
            "top_by_errors": [
                ...
                for endpoint, data in endpoints_by_errors
    ...
    async def _get_system_overview(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "load_average": (
                    list(psutil.cpu_times()) if hasattr(psutil, "cpu_times") else []
                ),
            }
        except Exception:
            return {}
    ...
    def _create_performance_widgets(
        ...
        response_status = (
            ...
            else "warning" if avg_response_time < 2000 else "critical"
        )
        widgets.append(
            DashboardWidget(
    ...
    def _create_error_widgets(
        ...
        error_status = (
            "healthy"
            if total_errors == 0
            else "warning" if total_errors < 10 else "critical"
        )
        widgets.append(
            DashboardWidget(
                id="total_errors",
    ...
    def _create_performance_summary(
        self, performance_data: dict[str, Any], error_data: dict[str, Any]
    ) -> dict[str, Any]:
        """Create performance summary for dashboard"""
        return {
            "overall_health": (
                "good" if performance_data.get("error_rate", 0) < 1 else "fair"
            ),
            "key_metrics": {
                "uptime": "99.9%",  # Would calculate from actual uptime data
                "availability": "99.8%",  # Would calculate from health checks
                "performance_score": max(
                    0, 100 - (performance_data.get("error_rate", 0) * 10)
                ),
    ...
    def _generate_recommendations(
        ...
        if performance_data.get("avg_response_time", 0) > 1000:
            recommendations.append(
                "Response times are elevated - check for bottlenecks"
            )
    ...
    def _aggregate_metrics_by_time(
        self, metrics: list[dict], time_buckets: list[datetime]
    ) -> list[float]:
        ...
        for i, bucket in enumerate(time_buckets):
            ...
            bucket_end = (
                bucket + timedelta(hours=1) if i < len(time_buckets) - 1 else utc_now()
            )
            ...
            bucket_values = [
                ...
                for m in metrics
                if bucket
                <= datetime.fromisoformat(m.get("timestamp", "").replace("Z", ""))
                < bucket_end
            ]
    ...
    def _get_response_time_chart_data(
        self, time_buckets: list[datetime]
    ) -> list[float]:
