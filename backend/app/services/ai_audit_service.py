...
class AIAuditService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            print(
                f"⚠️ AI Audit Service: Redis connection failed, using memory fallback: {e}"
            )
            self.redis_client = None
    ...
    async def log_ai_interaction(
        ...
        **kwargs,
        ...
        try:
            ...
            contains_pii = self._detect_pii(prompt) or (
                response and self._detect_pii(response)
            )
            ...
            if not self.redis_client and not self.db_session:
                # Direct lightweight memory path for tests (no I/O)
                if len(self.memory_cache) >= self.cache_max_size:
                    try:
                        ...
                        pass
                data = asdict(entry)
                if isinstance(data.get("compliance_level"), ComplianceLevel):
                    data["compliance_level"] = data["compliance_level"].value
                # Normalize timestamp to ISO string for filtering logic
                if isinstance(data.get("timestamp"), datetime):
                    data["timestamp"] = data["timestamp"].isoformat()
                self.memory_cache[entry.id] = data
            else:
    ...
    async def log_security_event(
        ...
        try:
            ...
            if not self.redis_client and not self.db_session:
                ...
                self.memory_cache[audit_id] = data
            else:
                await self._store_audit_entry(entry)
            return audit_id
    ...
    async def get_user_audit_trail(
        ...
        try:
            ...
            return await self._get_cached_audit_trail(
    ...
    async def get_compliance_report(
        ...
        try:
            ...
            if self.db_session:
                ...
                # Memory summary aggregation for tests running without DB
                interactions = list(self.memory_cache.values())
                report["summary"]["total_interactions"] = len(interactions)
                report["summary"]["pii_detected"] = sum(
                    1 for e in interactions if e.get("contains_pii")
                )
                report["summary"]["safety_filtered"] = sum(
                    1 for e in interactions if e.get("safety_filtered")
                )
                report["summary"]["rate_limited"] = sum(
                    1 for e in interactions if e.get("rate_limited")
                )
                # Provider counts
                ...
                report["providers"].update(provider_counts)
    ...
    async def cleanup_expired_logs(self) -> int:
        ...
        try:
            cutoff_date = utc_now() - timedelta(days=self.default_retention_days)
            deleted_count = 0
                ...
                async with self.db_session() as session:
                    result = await session.execute(
                        select(AIAuditLog).where(
                            or_(
                                AIAuditLog.retention_until < utc_now(),
                                and_(
            ...
            if self.db_session:
                async with self.db_session() as session:
                    log = AIAuditLog(
                        ...
                        retention_until=entry.timestamp
    ...
    async def _store_content(self, audit_id: str, prompt: str, response: str | None):
        ...
        try:
            ...
            if self.redis_client:
                await self.redis_client.setex(
                    f"content:{audit_id}", 86400, json.dumps(content_data)  # 24 hours
                )

        except Exception as e:
            print(f"❌ AI Audit Service: Failed to store content: {e}")

    async def _get_cached_audit_trail(
        self, user_id: str, start_date: datetime, end_date: datetime, limit: int
    ) -> list[dict[str, Any]]:
    ...
    def _generate_compliance_recommendations(self, report: dict[str, Any]) -> list[str]:
        ...
        if report["summary"]["pii_detected"] > 0:
            recommendations.append(
                "Consider implementing additional PII detection and masking"
            )

        if (
            report["summary"]["rate_limited"]
            > report["summary"]["total_interactions"] * 0.1
        ):
            recommendations.append(
                "High rate limiting detected - consider capacity planning"
            )

        if report["summary"]["safety_filtered"] > 0:
            recommendations.append(
                "Review safety filtering events for policy compliance"
            )
