...
class AnalyticsService:
    ...
    def __init__(self):
        """Initialize analytics service state."""
        # Service lifecycle / uptime tracking
    ...
    async def record_system_metrics(self, db: Session) -> dict[str, float]:
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"System metrics recorded: CPU {cpu_percent}%, Memory {memory.percent}%"
            )
            return metrics
    ...
    async def get_dashboard_data(self, db: Session, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            recent_metrics = (
                ...
                .filter(SystemMetrics.recorded_at >= cutoff_time)
            ...
            user_events = (
                ...
                .filter(AnalyticsEvent.created_at >= cutoff_time)
                .count()
            )
            ...
            # Active sessions
            active_sessions = (
                db.query(UserSession).filter(UserSession.is_active == True).count()
            )
            ...
            top_endpoints = (
                ...
                .group_by(AnalyticsEvent.endpoint)
                .order_by(desc("count"))
                .limit(10)
                .all()
            )
            ...
            for i in range(24):
                ...
                if hour_metrics:
                    cpu_values = [
                        ...
                        if m.metric_type == "cpu_usage"
                    ]
                    memory_values = [
                        m.metric_value
                        for m in hour_metrics
                        if m.metric_type == "memory_usage"
                    ]
            ...
            return {
                "system_metrics": {
                    "cpu_usage": (
                        recent_metrics[0].metric_value
                        if recent_metrics
                        and recent_metrics[0].metric_type == "cpu_usage"
                        else 0
                    ),
                    "memory_usage": next(
                        (
                    ...
                    "disk_usage": next(
                        (
                            m.metric_value
                            for m in recent_metrics
                            if m.metric_type == "disk_usage"
                        ),
                        0,
                    ),
                    "uptime_hours": (utc_now() - self.start_time).total_seconds()
                    / 3600,
                },
                "user_activity": {
                ...
                "api_metrics": {
                    ...
                    "success_rate": round(
                        ((api_calls - error_count) / max(api_calls, 1)) * 100, 2
                    ),
                },
                "top_endpoints": [
                    {
    ...
    async def get_real_time_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "uptime_seconds": int((utc_now() - self.start_time).total_seconds()),
    ...
    async def start_metrics_collection(
        self, db_session_factory, interval_seconds: int = 60
    ):
