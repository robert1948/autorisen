...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        "postgresql://autorisen:postgres@127.0.0.1:5433/autorisen", alias="DATABASE_URL"
    )
    ...
    model_config = SettingsConfigDict(
        env_file=[
            ".env.production" if os.getenv("ENVIRONMENT") == "production" else ".env"
        ],
...
if settings.is_production:
    if settings.secret_key == "dev-secret-key-change-in-production":
        raise ValueError(
            "CRITICAL: Production SECRET_KEY must be changed from development default!"
        )
    ...
    if (
        "localhost" in settings.cors_origins_list
        or "127.0.0.1" in settings.cors_origins_list
    ):
