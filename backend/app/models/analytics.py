...
class AnalyticsEvent(Base):
    ...
    )  # api_call, login, payment, etc.
    event_category = Column(
        String(50), nullable=False, index=True
    )  # system, user, payment, api
...
class SystemMetrics(Base):
    ...
    metric_type = Column(
        String(50), nullable=False, index=True
    )  # cpu, memory, response_time, etc.
    metric_value = Column(Float, nullable=False)
    metric_unit = Column(String(20), nullable=False)  # percent, ms, bytes, etc.
    source = Column(
        String(50), nullable=False, default="localhost"
    )  # localhost, heroku, etc.
    metric_metadata = Column(
        JSON, nullable=True
    )  # Additional metric details (renamed from metadata)
