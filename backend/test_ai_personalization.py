...
class TestAIPersonalizationService:
    ...
    def personalization_service(self):
        """Provide initialized service using asyncio.run for clean loop."""
    ...
    @pytest.mark.asyncio
    async def test_personality_profile_creation(
        self, personalization_service, sample_user_patterns, sample_user_prefs
    ):
    ...
    async def test_learning_style_inference(self, personalization_service):
        ...
        learning_style = personalization_service._infer_learning_style(
            visual_patterns, visual_prefs
        )
        ...
        kinesthetic_prefs = {}
        learning_style = personalization_service._infer_learning_style(
            kinesthetic_patterns, kinesthetic_prefs
        )
        ...
        reading_prefs = {}
        learning_style = personalization_service._infer_learning_style(
            reading_patterns, reading_prefs
        )
    ...
    async def test_communication_style_inference(self, personalization_service):
        ...
        prof_prefs = {}
        comm_style = personalization_service._infer_communication_style(
            prof_patterns, prof_prefs
        )
    ...
    async def test_expertise_level_inference(self, personalization_service):
        ...
        expertise = personalization_service._infer_expertise_level(
            beginner_patterns, beginner_prefs
        )
        ...
        expertise = personalization_service._infer_expertise_level(
            intermediate_patterns, intermediate_prefs
        )
        ...
        expertise = personalization_service._infer_expertise_level(
            advanced_patterns, advanced_prefs
        )
    ...
    async def test_prompt_personalization(self, personalization_service):
        ...
        personalized_prompt = await personalization_service.personalize_prompt(
            base_prompt=base_prompt, user_id=user_id, context_type="general_chat"
        )
        ...
        assert (
            "casual" in personalized_prompt.lower()
            or "friendly" in personalized_prompt.lower()
        )
    ...
    async def test_ai_parameter_adaptation(self, personalization_service):
        ...
        base_params = {"temperature": 0.7, "max_tokens": 1000, "model": "gpt-3.5-turbo"}
        ...
        # Verify adaptations
        assert (
            adapted_params["temperature"] > base_params["temperature"]
    ...
    async def test_profile_update_from_interaction(self, personalization_service):
        ...
        assert (
            updated_profile.confidence_score >= initial_profile.confidence_score
        )  # Allow same or higher
        assert "programming" in updated_profile.topics_of_interest
        assert (
            updated_profile.updated_at >= initial_profile.updated_at
        )  # Allow same or later
...
class TestPersonalizationAPI:
    ...
    async def test_personalized_chat_logic(self):
        ...
        assert (
            "personalized" in response.content.lower()
            or "visual" in response.content.lower()
        )
    ...
    async def test_multi_provider_personalization_integration(self):
        ...
        class MockEnhancedAI:
            async def generate_response(
                self, messages, user_id=None, use_personalization=True, **kwargs
            ):
...
async def run_personalization_tests():
    ...
    try:
        # Service tests
        await service_tests.test_personality_profile_creation(
            personalization_service, sample_patterns, sample_prefs
        )
        ...
        await service_tests.test_profile_update_from_interaction(
