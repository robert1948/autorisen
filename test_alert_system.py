    ...
    try:
        ...
        if alert_id:
            ...
            if resolve_success:
                ...
                print(
                    f"   Removed from active alerts: {alert_id not in alert_system.active_alerts}"
                )
        ...
        print(
            f"  High error rate data (15%): {'✅ Triggers' if should_trigger else '❌ No trigger'}"
        )

        # Test data that should not trigger
        normal_data = {"error_rate": 3.0}
        should_not_trigger = alert_system._evaluate_condition(
            high_error_rule, normal_data
        )
        print(
            f"  Normal error rate data (3%): {'❌ Triggers (unexpected)' if should_not_trigger else '✅ No trigger'}"
        )
