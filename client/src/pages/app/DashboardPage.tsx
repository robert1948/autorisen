...
function Sparkline({ points }: { points: DashboardTrendPoint[] }) {
  ...
  return (
    <div className="w-full">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto">
        ...
        {last ? (
          <circle cx={scaleX(points.length - 1)} cy={scaleY(last.value)} r="4" fill="rgba(120,200,255,0.95)" />
        ) : null}
      </svg>
...
function StatusPill({ status }: { status: DashboardSummaryActivityItem["status"] }) {
  const cls =
    status === "ok"
...
export default function DashboardPage() {
  ...
  const [meta, setMeta] = useState<DashboardSummaryMeta>({ isFallback: false });
  ...
  const hasReal = useMemo(() => !meta.isFallback, [meta.isFallback]);

  return (
    <div className="min-h-[calc(100vh-0px)] bg-slate-950 px-4 py-6 md:px-8">
            ...
            {meta.isFallback ? (
              <div className="mb-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-200">
                ...
                <div className="mt-2 text-xs text-amber-200/70">
                  {meta.message ?? "Using fallback data — temporary service issue."}
                </div>
              </div>
            ) : null}
