...
async def debug_thread_creation():
    ...
    try:
        ...
        conversation = await manager.create_conversation(
            "debug_user", conversation_data
        )
        print(f"✅ Created conversation: {conversation.conversation_id}")

        # Add messages
        msg1 = await manager.add_message(
            conversation.conversation_id, MessageRole.USER, "First message"
        )
        msg2 = await manager.add_message(
            conversation.conversation_id, MessageRole.ASSISTANT, "Second message"
        )
        msg3 = await manager.add_message(
            conversation.conversation_id, MessageRole.USER, "Third message"
        )
        msg4 = await manager.add_message(
            conversation.conversation_id, MessageRole.ASSISTANT, "Fourth message"
        )
        ...
        print(
            f"✅ Retrieved updated conversation with {len(updated_conversation.messages)} messages"
        )
        ...
        thread = await updated_conversation.create_thread(
            "Debug Thread", message_ids, "debug"
        )
