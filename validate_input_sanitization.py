...
def test_basic_functionality():
    ...
    print(
        "   ✅ PASS"
        if result["sanitized"] == "Hello &lt;b&gt;world&lt;/b&gt;!"
        else "   ❌ FAIL"
    )
    ...
    print(
        "   ✅ PASS"
        if xss_removed and len(result["threats_detected"]) > 0
        else "   ❌ FAIL"
    )
    ...
    print(
        "   ✅ PASS" if pii_redacted and len(result["pii_found"]) > 0 else "   ❌ FAIL"
    )
...
def test_sanitization_levels():
    ...
    test_input = (
        "Visit <a href='javascript:alert(1)'>link</a> or email test@example.com"
    )
