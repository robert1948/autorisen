...
def test_ddos_protection_basic_functionality():
    ...
    try:
        ...
        print(
            f"Block duration: {ddos_middleware.ddos_config['block_duration']} seconds"
        )
        print(
            f"Reputation threshold: {ddos_middleware.ddos_config['reputation_threshold']}"
        )
...
def test_rate_limiting_integration():
    ...
    try:
        ...
        for endpoint_type, limits in expected_limits.items():
            ...
            minute_match = (
                actual_limits["calls_per_minute"] == limits["calls_per_minute"]
            )
            hour_match = actual_limits["calls_per_hour"] == limits["calls_per_hour"]

            status = "✅" if minute_match and hour_match else "❌"
            print(
                f"{status} {endpoint_type}: {actual_limits['calls_per_minute']}/min, {actual_limits['calls_per_hour']}/hour"
            )
...
def test_ddos_specific_features():
    ...
    try:
        ...
        ddos_middleware.update_ip_reputation(
            test_blocked_ip, "rate_limit", current_time
        )
